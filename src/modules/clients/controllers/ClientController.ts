...
class ClientController {
  public async create(
    ...
    if (!req.body || Object.keys(req.body).length === 0) {
      return res
        .status(400)
        .json({ error: "Corpo da requisição não está definido." });
    }
      ...
      if (err instanceof Error) {
        switch (err.message) {
          ...
          case "Invalid cpf format":
            return res.status(404).json({ message: "Invalid cpf format" });
          //break;
  ...
  public async list(req: Request, res: Response): Promise<Response> {
    ...
    const excluido = req.query.excluido
      ? new Date(req.query.excluido as string)
      : undefined;

    const validOrderByValues: Array<"fullName" | "email" | "cpf" | "excluido"> =
      ["fullName", "email", "cpf", "excluido"];

    let orderBy: Array<"fullName" | "email" | "cpf" | "excluido"> = [];

    if (typeof req.query.orderBy === "string") {
      if (
        validOrderByValues.includes(
          req.query.orderBy as "fullName" | "email" | "cpf" | "excluido"
        )
      ) {
        orderBy.push(
          req.query.orderBy as "fullName" | "email" | "cpf" | "excluido"
      ...
      orderBy = req.query.orderBy.filter((item) =>
        validOrderByValues.includes(
          item as "fullName" | "email" | "cpf" | "excluido"
        )
      ) as Array<"fullName" | "email" | "cpf" | "excluido">;
    }

    const clients = await ListClientService.listClient({
      nome,
      email,
      cpf,
      excluido,
  ...
  public async update(
    ...
    try {
      const client = await UpdateClientService.updateClient({
        id,
        fullName,
        email,
        cpf,
